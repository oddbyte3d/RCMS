...
class XML_HTMLRenderer < OutputRenderer
    ...
    #TO-DO: make private
    private def getTemplate(theme)
        #puts "Theme : #{theme}"
        tmpTheme = theme
        themeFound = false
        while(tmpTheme != "")

            #puts "myPropertyLoader :  #{@myPropertyLoader.getProperties("TemplateDirectory")[tmpTheme]}"
    ...
    def loadPageContent_2(pageToLoad, parentPage)
        #puts "---------------loadPageContent_2---------------------"
        #myForward = "#{@baseDocRootInclude}#{@myPropertyLoader.getProperties("redirect").getProperty("XML")}xmlFile=#{pageToLoad}&pageLoaded=#{parentPage}"
        ...
        #request, response, session, properties, xmlFileToRender, theme, baseDocRoot,
        #                  baseDocRootInclude
        #puts "-------loaded page : #{pageToLoad} : parentPage : #{parentPage}--------------"
        #return "-------loaded page : #{pageToLoad} : parentPage : #{parentPage}--------------"
    end
