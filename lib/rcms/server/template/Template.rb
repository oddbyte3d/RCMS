...
class Template
    ...
    def initialize(templateDir, renderPage)
            ...
            if(File.exist?(config))
                ...
                if(@myConfig.key?("module_templates"))
                    ...
                    if(File.exist?(modConfPath) && !File.directory?(modConfPath))
                        ...
                        #puts "loaded module templates: #{@myModuleTemplates.keys}"
                    end
                end
            end
        end
    ...
    def getParsedTemplate
        ...
        if(File.exist?(@myTemplateDir) && File.directory?(@myTemplateDir))
            ...
            if(@myConfig != nil)
                ...
                pagePath = @myPageToRender.webPath
                #puts "Page Path : #{pagePath}"
                pageSpecific = ""
                start = 0
                if(@myConfig.key?("_#{start}#{pagePath}"))
                    #puts "setting pageSpecific"
                    pageSpecific = pagePath
                end
                #lookFor = "_#{start}#{pageSpecific}"
                #puts "-->.5  ---#{lookFor}---    #{@myConfig.key?(lookFor)}  "


                while(@myConfig.key?("_#{start}#{pageSpecific}"))

                    if(@myConfig["_#{start}#{pageSpecific}"] == "CONTENT")
                        @myParsedTemplate.concat("\n*CONTENT*\n")
                        #puts "-->2 : #{@myParsedTemplate}"
                    else
                      #puts "-->3 : #{@myConfig}"
