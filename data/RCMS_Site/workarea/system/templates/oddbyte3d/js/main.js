 ...
 */
{
    ...
    class Slideshow {
        constructor(el) {
            ...
            this.current = 0;
            this.uncoverItems = [];
            this.DOM.slides.forEach((slide,pos) => this.uncoverItems.push(new Uncover(slide.querySelector('.slide_image'), uncoverOpts[pos])));
            this.init();
        }
        init() {
            this.isAnimating = true;
            this.DOM.slides[this.current].classList.add('slide_c');
        ...
        navigate(pos) {
            ...
            this.uncoverItems[this.current].hide(true).then(() => {
                this.DOM.slides[this.current].classList.remove('slide_c');
                this.current = pos;
	...
	imagesLoaded(document.querySelectorAll('.slide_image'), {background: true}, () => {
        document.body.classList.remove('loading');

        const slideshow = new Slideshow(document.querySelector('.slides'));

        const pagin = document.querySelector('.pagin');
        const triggers = Array.from(pagin.querySelectorAll('.pagin_item'));
        triggers.forEach((trigger,pos) => {
            if ( pos === 0 ) {
                trigger.classList.add('pagin_item--current');
            }
            trigger.addEventListener('click', () => {
                if ( slideshow.isAnimating ) return;
                slideshow.navigate(pos);
                pagin.querySelector('.pagin_item--current').classList.remove('pagin_item--current');
                trigger.classList.add('pagin_item--current');
            })
        });
        ...
        document.addEventListener('keydown', (ev) => {
            ...
            pagin.querySelector('.pagin_item--current').classList.remove('pagin_item--current');
            triggers[newpos].classList.add('pagin_item--current');
        });
    });
}
